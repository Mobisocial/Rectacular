...
public abstract class ManagerBase {
    ...
    void bindField(SQLiteStatement statement, int field, Object val) {
        if (val == null) {
            ...
            statement.bindLong(field, Long.valueOf((Integer)val));
            ...
            statement.bindDouble(field, Double.valueOf((Float)val));
        } else if (val instanceof Boolean) {
            statement.bindLong(field, ((Boolean)val) ? 1 : 0);
        } else if (val instanceof Uri) {
            statement.bindString(field, ((Uri)val).toString());
        } else {
